...
// NewLoggerWithWriter builds a logger that writes to w (tests/helpers).
func NewLoggerWithWriter(w io.Writer, settings LoggerSettings) *Logger {
	if w == nil {
		w = io.Discard
	}
	return &Logger{
...
func NewDiscardLogger() *Logger {
	return NewLoggerWithWriter(io.Discard, LoggerSettings{})
}
...
// SanitizeFilename replaces path-unsafe characters for log/cache filenames.
func SanitizeFilename(name string) string {
