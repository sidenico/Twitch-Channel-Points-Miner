...
func (t *TwitchLogin) runDeviceFlow() error {
	...
	addDeviceHeaders := func(req *http.Request) {
		...
		ua := t.UserAgent
		if ua == "" {
			ua = "Mozilla/5.0 (Linux; Android 7.1; Smart Box C1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36"
		}
...
// NewTwitchLoginWithIdentity builds a TwitchLogin with pre-set token/user id (tests/helpers).
func NewTwitchLoginWithIdentity(token, userID string) *TwitchLogin {
	return &TwitchLogin{Token: token, userID: userID}
}
