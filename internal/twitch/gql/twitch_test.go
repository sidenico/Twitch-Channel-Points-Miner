...
func newTestTwitch(t *testing.T, responder func(operation string) (*http.Response, error)) *Twitch {
	...
	return &Twitch{
		...
		twitchLogin:    func() *auth.TwitchLogin { l := &auth.TwitchLogin{Token: "token"}; l.SetUserID("user-id"); return l }(),
		client:         client,
	}
}

func newTestStreamer(watchStreak bool) *streamer.Streamer {
...
func TestLoadChannelPointsContextWrapsMissingChannel(t *testing.T) {
	twitch := &Twitch{
		...
		versionTTL:     time.Hour,
		versionFetched: time.Now(),
		twitchLogin:    &auth.TwitchLogin{Token: "token"},
		client: &http.Client{
			Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
