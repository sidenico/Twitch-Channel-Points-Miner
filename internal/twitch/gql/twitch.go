...
type Twitch struct {
	...
	onGameChange   func(streamer *streamer.Streamer, previous, current string)
}
...
func (t *Twitch) UserID() string {
	if t == nil || t.twitchLogin == nil {
		return ""
	}
	return t.twitchLogin.UserID()
}

func (t *Twitch) AuthToken() string {
	if t == nil || t.twitchLogin == nil {
		return ""
	}
...
func (t *Twitch) Username() string {
	...
	return strings.TrimSpace(t.twitchLogin.Username)
}
...
func NewTwitch(username, userAgent, password string, logger debugLogger, anonymizer *privacy.Anonymizer) (*Twitch, error) {
	...
	login, err := auth.NewTwitchLogin(constants.ClientID, deviceID, username, userAgent, password)
	if err != nil {
		return nil, err
	}
...
func (t *Twitch) SetGameChangeHandler(handler func(streamer *streamer.Streamer, previous, current string)) {
...
func (t *Twitch) GetFollowers(limit int, order streamer.FollowersOrder) ([]string, error) {
...
func (t *Twitch) LoadChannelPointsContext(streamer *streamer.Streamer) (int, error) {
...
func (t *Twitch) CheckStreamerOnline(streamer *streamer.Streamer) (bool, error) {
	...
	if streamer.Stream == nil {
		streamer.Stream = newEmptyStream()
	}
...
func (t *Twitch) streamInfo(streamer *streamer.Streamer) (*streamInfoResult, error) {
...
func (t *Twitch) UpdateStream(streamer *streamer.Streamer) error {
	if streamer.Stream == nil {
		streamer.Stream = newEmptyStream()
	}
...
func (t *Twitch) GetSpadeURL(streamer *streamer.Streamer) error {
	if streamer.Stream == nil {
		streamer.Stream = newEmptyStream()
	}
	headers := map[string]string{
		"User-Agent": UserAgents["Linux"]["FIREFOX"],
	}
...
func (t *Twitch) SendMinuteWatched(streamer *streamer.Streamer) error {
...
func (t *Twitch) ClaimBonus(streamer *streamer.Streamer, claimID string) error {
	if claimID == "" {
...
func (t *Twitch) claimBonusTV(streamer *streamer.Streamer, claimID string) error {
	if streamer == nil || streamer.ChannelID == "" {
...
func (t *Twitch) ClaimMoment(streamer *streamer.Streamer, momentID string) error {
	if momentID == "" {
		return nil
	}
...
func (t *Twitch) JoinRaid(streamer *streamer.Streamer, raidID string) error {
	if raidID == "" {
...
func (t *Twitch) MakePrediction(event *prediction.PredictionEvent) error {
...
func (t *Twitch) ContributeToCommunityGoals(streamer *streamer.Streamer) {
...
func (t *Twitch) ContributeToCommunityGoal(streamer *streamer.Streamer, goalID, title string, amount int) error {
	if amount <= 0 || goalID == "" {
		return nil
	}
...
func (t *Twitch) CampaignIDsForStreamer(streamer *streamer.Streamer) ([]string, error) {
...
func parseCommunityGoals(goals interface{}) map[string]*streamer.CommunityGoal {
	arr, ok := goals.([]interface{})
	if !ok {
		return nil
	}
	result := make(map[string]*streamer.CommunityGoal, len(arr))
	for _, raw := range arr {
		if goalMap, ok := raw.(map[string]interface{}); ok {
			if goal := streamer.NewCommunityGoalFromGQL(goalMap); goal != nil && goal.ID != "" {
				result[goal.ID] = goal
			}
		}
	}
...
func (t *Twitch) RecoverStreak(streamer *streamer.Streamer) (bool, error) {
	if streamer == nil || streamer.ChannelID == "" {
