...
func (p *PubSubClient) streamerName(streamer *streamer.Streamer) string {
	if streamer == nil {
		return ""
	}
	if p.anonymizeLogs() {
...
func NewPubSubClient(
	...
	streamers []*streamer.Streamer,
	disableSSL bool,
	onGain func(*streamer.Streamer, int, string, int),
	onPresence func(*streamer.Streamer, bool, string),
) *PubSubClient {
	streamerMap := make(map[string]*streamer.Streamer)
			...
			if msgType == "RESPONSE" && envelope != nil {
				if respErr, ok := envelope["error"].(string); ok && strings.TrimSpace(respErr) != "" {
					...
					if strings.Contains(respErr, "ERR_BADAUTH") {
						username := ""
						if p != nil && p.twitch != nil {
							username = strings.TrimSpace(p.twitch.Username())
						}
...
func (p *PubSubClient) listenTopics(conn *websocket.Conn, topics []string) error {
	...
	for _, t := range topics {
		...
		if needsAuth(t) {
			data["auth_token"] = p.twitch.AuthToken()
		}
...
func (p *PubSubClient) processPredictionChannel(topic string, payload map[string]interface{}) error {
	...
	case "event-created":
		...
		event := prediction.NewPredictionEvent(streamer, eventMap)
		if event == nil {
			return nil
		}
...
func (p *PubSubClient) processCommunityPointChannel(topic string, payload map[string]interface{}) error {
	...
	s := p.streamerMap[channelID]
	if s == nil || !s.Settings.CommunityGoals {
		return nil
	}
	msgType := strings.ToLower(fmt.Sprint(payload["type"]))
	if s.CommunityGoals == nil {
		s.CommunityGoals = make(map[string]*streamer.CommunityGoal)
	}
	switch msgType {
	case "community-goal-created", "community-goal-updated":
		data, _ := payload["data"].(map[string]interface{})
		goalData, _ := data["community_goal"].(map[string]interface{})
		if goal := streamer.NewCommunityGoalFromPubSub(goalData); goal != nil && goal.ID != "" {
			s.CommunityGoals[goal.ID] = goal
		}
		p.twitch.ContributeToCommunityGoals(s)
	case "community-goal-deleted":
		data, _ := payload["data"].(map[string]interface{})
		goalData, _ := data["community_goal"].(map[string]interface{})
		id := stringOrDefault(goalData["id"])
		if id != "" {
			delete(s.CommunityGoals, id)
		}
	}
	return nil
}
...
func (p *PubSubClient) logPredictionResult(event *prediction.PredictionEvent, result map[string]interface{}) {
...
func (p *PubSubClient) resolvePredictionFromChannel(event *prediction.PredictionEvent, eventMap map[string]interface{}) {
...
func payoutForOutcome(decision prediction.PredictionDecision, outcomes []prediction.PredictionOutcome, winningID string) int {
