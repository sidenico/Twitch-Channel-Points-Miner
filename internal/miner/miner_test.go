...
func testWatchSelectionStreakStreamer(username, channelID, game string, onlineAt time.Time, minuteWatched float64) *st.Streamer {
	return &st.Streamer{
		Username:  username,
		ChannelID: channelID,
		IsOnline:  true,
		OnlineAt:  onlineAt,
		Settings: st.StreamerSettings{
			WatchStreak: true,
		},
		Stream: &st.Stream{
...
func TestResolvedStreakShortRestartDoesNotPrioritizeStreak(t *testing.T) {
	now := time.Now()
	m := &Miner{logger: notify.NewDiscardLogger()}
	streamer := &st.Streamer{
		Username:      "restarted",
...
func TestLogDropStatusesHonorsProgressFlag(t *testing.T) {
	...
	m := &Miner{
		logger: notify.NewLoggerWithWriter(&buf, notify.LoggerSettings{}),
	}
...
func TestPickStreamersToWatchPrioritizesRestartedStreakCandidate(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{first, second, restarted})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
	if got[0] != restarted {
...
func TestPickStreamersToWatchKeepsInProgressStreakSlotsWhenHigherPriorityStreakAppears(t *testing.T) {
	...
	initial := m.pickStreamersToWatch([]*st.Streamer{first, second})
	if len(initial) != 2 {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{first, second, higherPriority})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
	if got[0] != first || got[1] != second {
...
func TestPickStreamersToWatchKeepsSameGameStreakAheadOfFallback(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{active, sameGameStreak, fallback})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
...
func TestPickStreamersToWatchForceStreakSlotIgnoresGameDiversity(t *testing.T) {
	...
	subscribed := &st.Streamer{
		Username: "subscribed",
		IsOnline: true,
		OnlineAt: onlineAt,
		ActiveMultipliers: []st.ActiveMultiplier{
			{Factor: 1.2},
		},
		Stream: &st.Stream{
	...
	got := m.pickStreamersToWatch([]*st.Streamer{subscribed, sameGameStreak, fallback})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
...
func TestStreamerInitialPointsKeyUsesChannelIDWhenAvailable(t *testing.T) {
	streamer := &st.Streamer{Username: "oldlogin", ChannelID: "123456"}
...
func TestPickStreamersToWatchSkipsBackedOffStreamer(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{backedOff, healthy, fallback})
...
func TestShouldClaimDrops(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			global: false,
			streamers: []*st.Streamer{
				{Settings: st.StreamerSettings{ClaimDrops: false}},
				{Settings: st.StreamerSettings{ClaimDrops: true}},
			},
			want: true,
		},
		{
