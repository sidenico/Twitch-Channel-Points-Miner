...
type (
	streamData       = streamer.Stream
	streamerModel    = streamer.Streamer
	streamerSettings = streamer.StreamerSettings
	activeMultiplier = streamer.ActiveMultiplier
)

func testWatchSelectionStreakStreamer(username, channelID, game string, onlineAt time.Time, minuteWatched float64) *streamerModel {
...
func TestResolvedStreakShortRestartDoesNotPrioritizeStreak(t *testing.T) {
	now := time.Now()
	m := &Miner{logger: notify.DiscardLogger()}
	streamer := &streamer.Streamer{
		Username:      "restarted",
...
func TestLogDropStatusesHonorsProgressFlag(t *testing.T) {
	...
	m := &Miner{
		logger: notify.NewLoggerWithOutput(notify.LoggerSettings{}, &buf),
	}
	statuses := []gql.DropStatus{
		{
...
func TestPickStreamersToWatchPrioritizesRestartedStreakCandidate(t *testing.T) {
	...
	first := &streamer.Streamer{
		Username:      "first",
		IsOnline:      true,
		OnlineAt:      onlineAt,
		ChannelPoints: 100,
	}
	second := &streamer.Streamer{
		Username:      "second",
		IsOnline:      true,
		OnlineAt:      onlineAt,
		ChannelPoints: 200,
	}
	restarted := &streamer.Streamer{
		Username:  "restarted",
		IsOnline:  true,
		OnlineAt:  onlineAt,
		OfflineAt: now.Add(-10 * time.Minute),
	...
	got := m.pickStreamersToWatch([]*streamerModel{first, second, restarted})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
	if got[0] != restarted {
...
func TestPickStreamersToWatchKeepsInProgressStreakSlotsWhenHigherPriorityStreakAppears(t *testing.T) {
	...
	initial := m.pickStreamersToWatch([]*streamerModel{first, second})
	if len(initial) != 2 {
	...
	got := m.pickStreamersToWatch([]*streamerModel{first, second, higherPriority})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
	if got[0] != first || got[1] != second {
...
func TestPickStreamersToWatchKeepsSameGameStreakAheadOfFallback(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*streamerModel{active, sameGameStreak, fallback})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
...
func TestPickStreamersToWatchForceStreakSlotIgnoresGameDiversity(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*streamerModel{subscribed, sameGameStreak, fallback})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
...
func TestStreamerInitialPointsKeyUsesChannelIDWhenAvailable(t *testing.T) {
	streamer := &streamer.Streamer{Username: "oldlogin", ChannelID: "123456"}
...
func TestShouldRepairStreamerIdentityOnlyForChannelNotFound(t *testing.T) {
	if !shouldRepairStreamerIdentity(fmt.Errorf("wrapped: %w", gql.ErrChannelNotFound)) {
		t.Fatalf("ErrChannelNotFound should trigger identity repair")
	}
	if shouldRepairStreamerIdentity(gql.ErrStreamerOffline) {
...
func TestPickStreamersToWatchSkipsBackedOffStreamer(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*streamerModel{backedOff, healthy, fallback})
...
func TestShouldClaimDrops(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			global: false,
			streamers: []*streamerModel{
				{Settings: streamerSettings{ClaimDrops: false}},
				{Settings: streamerSettings{ClaimDrops: true}},
			},
			want: true,
		},
		{
