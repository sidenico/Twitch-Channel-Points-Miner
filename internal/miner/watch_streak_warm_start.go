...
func sameWarmStartStream(entry watchStreakWarmStartEntry, streamer *streamer.Streamer) bool {
...
func (c *watchStreakWarmStartCache) resolvedEntryForStreamer(streamer *streamer.Streamer, now time.Time) (watchStreakWarmStartEntry, bool) {
...
func (c *watchStreakWarmStartCache) updateFromStreamer(streamer *streamer.Streamer, checkedAt time.Time) {
	if c == nil || streamer == nil {
		return
	}
	if checkedAt.IsZero() {
...
func (c *watchStreakWarmStartCache) saveIfDirty() error {
	...
	if err := persistence.SaveJSON(c.path, watchStreakWarmStartFile{
