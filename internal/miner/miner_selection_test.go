...
func onlineCandidate(username string, points int, game string, onlineAt time.Time) *st.Streamer {
	return &st.Streamer{
		Username:      username,
...
func usernames(streamers []*st.Streamer) []string {
...
func TestPickStreamersToWatchDropsPriority(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{without, withDrops, third})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
...
func TestPickStreamersToWatchSkipsOnlineGracePeriod(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{fresh, ready})
...
func TestPickStreamersToWatchSkipsOfflineAndExcludedGames(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{offline, excluded, ok})
...
func TestPickStreamersToWatchGameDiversityPrefersDifferentGames(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{a1, a2, b1})
	if len(got) != 2 {
		t.Fatalf("expected 2 watchers got %d", len(got))
	}
...
func TestPickStreamersToWatchDefersStreakWithoutPriorityGame(t *testing.T) {
	...
	got := m.pickStreamersToWatch([]*st.Streamer{streak, orderFirst, orderSecond})
	if len(got) != 2 {
...
func TestSetPresenceFirstKnownOnlineSetsTimestampAndLogsOnce(t *testing.T) {
	...
	s := &st.Streamer{
		Username:  "newbie",
		ChannelID: "1",
		Settings:  st.StreamerSettings{IRCMode: st.IRCModeNever},
		Stream:    st.NewStream(),
	}
...
func TestSetPresenceOfflineTransitionArmsResolvedStreakCarryover(t *testing.T) {
	...
	s := &st.Streamer{
		...
		Settings:             st.StreamerSettings{WatchStreak: true, IRCMode: st.IRCModeNever},
		Stream: &st.Stream{
